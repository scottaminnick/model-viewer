  ...
  def get_contour_overlay(self, cycle_dt, fxx) -> dict | None:
        ...
        try:
            ds250_z = herbie_fetch(
                ...
                cycle_dt, fxx,
                [":HGT:250 mb:"],
                f"{tag}_z250",          # same subdir → already cached from get_values()
            )
            z250   = extract_var(ds250_z, ["gh", "z", "hgt", "HGT", "HGHT"])
            lat2d, lon2d = get_latlon(ds250_z)
            # Convert metres → hundreds of feet for conventional height labels
            z250_100ft = z250 * 3.28084 / 100.0
            return {
                "lat2d":      lat2d,
                "lon2d":      lon2d,
                "data":       z250_100ft,
                "levels":     list(np.arange(310, 420, 6)),  # 310–420 dam, every 6
                "color":      "#1a1a1a",
                "linewidths": 1.1,
                "alpha":      0.80,
                "label_fmt":  "%i",
